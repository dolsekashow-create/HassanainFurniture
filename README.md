...
- **Phone / tablet** (`max-width: 1023px` or no hover) → tapping the button
  opens the PDF in a new browser tab **and** reveals a full-bleed swipeable page
  viewer built from pre-rendered images in `public/catalogue/pages/` (47 pages,
  lazy loaded, page counter, prev/next arrows). Tap any page to open it full
  screen.
...
The footer ends with a "Developed by Zero—Nine" link to
<https://www.zero--nine.online/>, styled after the credit on
abdullah-mubarak.com. Wording lives under `footer.developedBy` in
`lib/content.ts`.

The logo is at `public/images/zero-nine.webp` (a `.png` copy sits beside it).
The black backdrop has been cut out, so the gold emblem sits cleanly on the dark
footer.

## To verify

- The hero backdrop is the dining-table photo; the two inset photos use the
  other showroom shots so the same picture isn't repeated. Change the `src`
  values in `components/hero-section.tsx` if you'd prefer it twice.
