...
/**
 * Phones and tablets can't render a PDF inside <object>/<iframe>, so those get
 * the image-based page viewer instead. Desktop keeps the native PDF viewer.
 */
function useNeedsImageViewer() {
  const [needsImages, setNeedsImages] = useState(false)

  useEffect(() => {
    const query = window.matchMedia('(max-width: 1023px), (hover: none)')
    const update = () => setNeedsImages(query.matches)
    update()
...
export function CatalogueSection() {
  ...
  const needsImageViewer = useNeedsImageViewer()
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <button
                ...
                onClick={() => {
                  const next = !open
                  setOpen(next)
                  // On phones/tablets, also hand the PDF to the browser's own
                  // viewer in a new tab — the in-page image viewer still opens
                  // below either way.
                  if (next && needsImageViewer) {
                    window.open(CATALOGUE_PDF, '_blank', 'noopener,noreferrer')
                  }
                }}
